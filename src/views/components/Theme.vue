	...
	<v-container class="container--fluid grid-list-md">
		<v-row no-gutters>
			<v-col cols="12">
				<v-alert :value="true" color="info" outlined dense>
					...
					<a class="link-type" href="//theme-generator.vuetifyjs.com">Theme Generator</a>
				</v-alert>
			</v-col>

			<v-col md="4" offset-md="4" sm="8" offset-sm="2" cols="12">
				<app-widget :title="$t('ui.theme')" class="text-center" icon="mdi-palette" padding-hide>
		...
		<v-row align="center">
			<v-col cols="6" md="2" class="text-center">
				<v-col v-for="item in items" :key="item.color" cols="12">
			...
			<v-col cols="6" md="2" class="text-center">
				<v-switch v-for="item in items" :key="item.color" v-model="turn" :color="item.color" :label="item.color" hide-details />
			</v-col>
			<v-col cols="6" md="4">
				<v-alert v-for="item in items" :key="item.color" :color="item.color" :value="true" :icon="item.icon" outlined dense>
					This is a {{ item.color }} outline alert.
				</v-alert>
			</v-col>
			<v-col cols="6" md="4">
				<v-alert v-for="item in items" :key="item.color" :color="item.color" :value="true" :icon="item.icon" dense>
