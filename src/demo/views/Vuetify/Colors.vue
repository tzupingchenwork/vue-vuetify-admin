<template>
	<v-container class="container--fluid">
		<v-row>
			<v-col cols="12">
				<v-text-field v-model="search" prepend-icon="mdi-filter" solo="solo" label="Search Materil Colors" single-line="single-line" />
			</v-col>
			<v-col v-for="(color, key) in computedColors" :key="key" cols="12" sm="6" md="3">
...
export default {
	...
	methods: {
		...
		getColorClass(key) {
			if (['white', 'transparent', 'shades'].includes(key) || key.indexOf('lighten') > -1 || key.indexOf('accent') > -1)
				return 'black--text';
			return 'white--text';
		}
	}
